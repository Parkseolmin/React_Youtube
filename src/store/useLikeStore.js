...
const useLikeStore = create((set, get) => ({
  ...
  toggleLike: async (userId, videoId, videoInfo) => {
    if (!userId) return;
    // userId가 없으면 아무 작업도 하지 않습니다.
    set({ isLoading: true });
    // isLoading 상태를 true로 설정하여 로딩 중임을 나타냅니다.
    const likeRef = doc(db, 'likes', `${userId}_${videoId}`);
    // Firestore에서 해당 사용자가 해당 동영상에 좋아요를 눌렀는지 확인하기 위해 문서 참조를 생성합니다.
    const currentLikeStatus = get().likedVideos[videoId];
    // 현재 likedVideos 상태에서 해당 동영상의 좋아요 상태를 가져옵니다.

    if (currentLikeStatus) {
      await deleteDoc(likeRef);
      // 이미 좋아요 상태라면 Firestore에서 해당 문서를 삭제하여 좋아요를 취소합니다.
    } else {
      ...
      // 좋아요 상태가 아니라면 Firestore에 새 문서를 생성하여 좋아요를 설정합니다.
    }
    ...
    // likedVideos 상태를 업데이트하여 해당 동영상의 좋아요 상태를 토글합니다.
    // isLoading 상태를 false로 설정합니다.
  },
  ...
  fetchLikedVideos: async (userId) => {
    ...
    // Firestore에서 'likes' 컬렉션 참조를 생성합니다.
    const q = query(likesRef, where('userId', '==', userId));
    // 해당 사용자가 좋아요 한 동영상들을 조회하기 위해 쿼리를 생성합니다.
    const querySnapshot = await getDocs(q);
    // 쿼리 결과를 가져옵니다.
    console.log('쿼리결과::', querySnapshot);
    const videos = querySnapshot.docs.map((doc) => doc.data().videoInfo);
    // 쿼리 결과에서 각 문서의 videoInfo를 추출하여 동영상 목록을 만듭니다.
    const likedVideosMap = videos.reduce((acc, video) => {
      acc[video.id] = true;
      return acc;
    }, {});
    // 동영상 목록을 순회하여 likedVideos 맵을 생성합니다. 각 동영상의 ID를 키로 하고 값을 true로 설정합니다.
    set({ likedVideos: likedVideosMap, isLoading: false });
    // likedVideos 상태를 업데이트하고 isLoading 상태를 false로 설정합니다.
    return videos;
    // 동영상 목록을 반환합니다.
  },
}));
